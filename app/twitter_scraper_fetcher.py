...
def get_elements(twitter_handle):
  ...
  repsonse = request.get(url,header = {'User-Agent' : AGENTS})
...
def get_user_tweets(twitter_handle):
  ...
  for post in elements:
    for text in post.contents:
      # check if line contains real text/not empty
...
def clean_tweets_data(tweets):
  ...
  for tweet in tweets:
    ...
    cleaned_tweets.appent(mentions_pattern.sub(r"",text_without_url))
